...
class Quad_env():
    ...
    def step(self,t_):
        ...
        if norm(dx)>0.5:
            psiRef = arctan2(dx[0],dx[1])
            while psiRef-self.desired_x[0][2]>np.pi:
                psiRef-=2*np.pi
            while psiRef-self.desired_x[0][2]<-np.pi:
                psiRef+=2*np.pi
        else:
            psiRef = self.desired_x[0][2]
        xRef = self.desired_x[0]
        xRef[2] = psiRef
...
def sim(mpc):
    ...
    state_rec,input_rec,backup_rec,backup_choice_rec,xPred_rec,zPred_rec = Robot_sim(env,14)
    pdb.set_trace()
